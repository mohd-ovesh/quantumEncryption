...
#     """
#     Receive next message. If it's MSG_ABORT, print reason and return None.
#     Otherwise return the message dict.
#     """
#     msg = receive_message(sock)
#     if msg['type'] == MSG_ABORT:
#         reason = msg['payload'].get('reason', 'Unknown')
#         print(f"\n  ❌ [{step_name}] Alice aborted: {reason}")
#         logger.log_security_event('ALICE_ABORT', f"{step_name}: {reason}", 'CRITICAL')
#         return None
#     return msg
...
#     """
#     Bob's E91 Protocol — Clean version.

#     Message receive order (must match Alice's send order exactly):
#     ─────────────────────────────────────────────────────────────
#     1. MSG_BASIS_COMPARE    ← alice_bases
#     3. MEASUREMENT_RESULTS  ← bob_bits_all
#     4. MSG_CHSH_RESULT      ← S_value, correlators
#        [possible MSG_ABORT if CHSH failed]
#     5. MSG_ERROR_RATE       ← alice_key_bits
#        [possible MSG_ABORT if not enough bits]
#     6. MSG_KEY_HASH         ← hash of Alice's final key
#        [possible MSG_ABORT if key mismatch]
#     7. MSG_READY
#     """
#     global logger
...
#     # ── Step 1: Receive Alice's bases ───────────────────────────────
#     print(f"\n  📥 [1/7] Receiving Alice's basis choices...")
...
#     print(f"  ✅ Bob's bases: {bob_bases[:12]}...")

#     # ── Step 3: Send bases to Alice ─────────────────────────────────
#     print(f"\n  📤 [3/7] Sending bases to Alice...")
...
#     # ── Step 7: Receive key material from Alice ──────────────────────
#     # Alice sends alice_key_bits directly — no QBER check needed.
#     # CHSH already confirmed security.
#     print(f"\n  🧮 [7/7] Receiving key bits from Alice...")
...
#     print(f"     Total matching-basis pairs : {n_key_bits}")
#     print(f"     Key bits received          : {len(alice_key_bits)}")
#     print(f"     CHSH S-value               : {chsh_s:.4f} (confirmed secure)")

#     if len(alice_key_bits) < 8:
#         print(f"\n  ❌ Not enough key bits ({len(alice_key_bits)})")
#         logger.log_security_event('INSUFFICIENT_BITS', str(len(alice_key_bits)), 'CRITICAL')
#         return None

#     # Generate SAME key as Alice
#     # Both use: SHA-256(alice_key_bits) → same 128-bit key
#     kg        = KeyGenerator()
...
#     else:
#         print(f"\n  ❌ Keys DO NOT match!")
#         print(f"     Alice: {alice_key_hash[:32]}...")
#         print(f"     Bob  : {our_hash[:32]}...")
#         logger.log_security_event('KEY_MISMATCH', 'Keys differ', 'CRITICAL')
#         return None

#     receive_message(sock)                              # Alice's MSG_READY
...
#                 clear_line()
#                 print("\n\n  [Alice disconnected]")
#                 logger.log_security_event('PEER_DISCONNECT', 'Alice left', 'INFO')
...
#     SERVER_IP = '192.168.29.42'   # ← Alice's IP
...
#     print("  Commands: 'exit' to quit | 'status' for session stats")
#     print("═"*62 + "\n")
...
def main():
    ...
    SERVER_IP = '192.168.29.42'   # ← Change to server PC's IP
    ...
    print(f"\n  Hello, {my_name}! Connecting to {SERVER_IP}:12345 ...")
